...
const store = async () => {
  ...
  // The store appends "/api" itself, so keep the saved value free of it and of
  // any trailing slash the user typed.
  const connectionString = String(Config.value.connectionString ?? "")
    .trim()
    .replace(/\/+$/, "");
  const organization = String(Config.value.organization ?? "").trim();

  if (!connectionString) {
    showErrorToast("Connection string is required");
    return;
  }

  loading.value = true;
  try {
    await useConfigStore().store(connectionString, organization);
    await Swal.fire({
      icon: "success",
  ...
  await useConfigStore()
    ...
    .then(() => {
      if (
        ...
        // No "/api" here: the store appends it when building Api.defaults.baseURL.
        Config.value.connectionString = envConfig._baseURL;
      }
      if (
        Config.value.organization == null ||
    ...
    <IPageContent>
      ...
      <IRow cols-lg="2" cols-md="2" cols-sm="1">
        <ICol span="1" span-md="1" span-sm="1" class="flex">
          <div
            class="mb-2 md:text-sm text-base mr-3 font-bold mx-2 text-text dark:text-textLight"
          >
            {{ t("ConnectionString") }}
          </div>
          <input
      ...
      <IRow cols-lg="2" cols-md="2" cols-sm="1">
        <ICol span="1" span-md="1" span-sm="1" class="flex">
          <div
            class="mb-2 md:text-sm text-base mr-3 font-bold text-text dark:text-textLight mx-2"
          >
            {{ t("Organization") }}
          </div>
          <input
