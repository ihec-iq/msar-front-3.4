...
export function useValidation() {
  function sameAs(
    field2: { field: string; caption: string } = { field: "", caption: "" },
    options: { message: string } = {
      message:
        t("ValidationErrors.FieldNotSame") +
        " " +
      ...
      check: (field1: any) => {
        if (options.key == "") {
          ...
          if (
            ...
            field1 !== null &&
            field1.hasOwnProperty(options.key) &&
            options.key != ""
          ) {
  ...
  function string(
    options: { message: string } = {
      message: t("ValidationErrors.FieldMustBeString"),
    }
  ): IValidator {
    const validator: IValidator = {
      name: "string",
  ...
  function validate(
    ...
    try {
      let success = true;
      const errors: Array<IValidatorError> = [];

      validators.forEach((validator) => {
        const keyValue = object[validator.field];
        ...
        validator.rules.forEach((rule) => {
          ...
          if (rule.regexp != undefined) {
            const regex = new RegExp(rule.regexp as RegExp);
            ...
            if (rule.name == "sameAs") {
              const field2 = object[String(rule.field2)];
