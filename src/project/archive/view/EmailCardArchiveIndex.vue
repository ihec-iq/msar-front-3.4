...
const formattedDate = computed(() => {
  ...
  const sameDay = moment().format("YYYY-MM-DD") === moment(v).format("YYYY-MM-DD");
  const sameYear = moment().format("YYYY") === moment(v).format("YYYY");
  if (sameDay) return moment(v).format("h:mm A");
  if (sameYear) return moment(v).format("MMM D");
...
<template>
  <article
    class="w-full rounded-2xl border border-gray-200 bg-white transition-all duration-200 hover:shadow-sm
  ...
  >
    ...
    >
      <!-- العنوان -->
      <h3
        class="col-span-12 lg:col-span-6 font-bold text-lg leading-snug rtl:text-right ltr:text-left
    ...
    <footer class="flex flex-wrap items-center gap-2 px-4 pb-4">
      <!-- نوع الأرشيف -->
      <span
        class="inline-flex items-center rounded-full border border-orange-300 bg-orange-100 text-[12px]
      ...
      <span
        ...
        class="inline-flex items-center rounded-lg border border-gray-300 bg-gray-100
