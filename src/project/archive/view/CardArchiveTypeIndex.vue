...
const formattedCount = computed(() =>
  typeof props.count === "number" ? props.count.toLocaleString() : String(props.count)
);
  ...
  switch (props.color) {
    ...
    case "amber":
      return {
        ...
        badge: "bg-amber-50 text-amber-800 dark:bg-amber-500/10 dark:text-amber-300",
...
<template>
  <button
    ...
    :disabled="disabled"
    @click="handleClick"
    class="group w-full text-start rounded-lg border border-gray-200 dark:border-gray-700
           bg-white/70 dark:bg-gray-900/50 backdrop-blur-sm
           p-3 sm:p-4 shadow-sm transition duration-200
  ...
  >
    <div class="flex items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-3 min-w-0">
        ...
        <h3 class="truncate text-sm sm:text-base font-medium text-gray-900 dark:text-gray-100">
          {{ title }}
        </h3>
      </div>
