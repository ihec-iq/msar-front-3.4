...
const { overview, loading, fetchOverview, invalidateCache } = useDashboard();
    ...
    <div v-else-if="overview" class="space-y-6">
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          ...
          <StatCard
            ...
            :value="overview.employees.inactive"
            icon="mdi-account-off"
            color="red"
          />
          <StatCard
            title="الأقسام"
            :value="overview.employees.by_section.length"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <ChartCard title="الموظفون حسب القسم">
            <BarChart
              :data="overview.employees.by_section.slice(0, 10)"
              horizontal
            />
          </ChartCard>
        </div>
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            title="الموظفون مع إجازات"
            :value="overview.vacations.total_employees_with_vacation"
            icon="mdi-calendar-check"
            color="blue"
          />
          <StatCard
            title="إجمالي أيام الإجازات"
            :value="overview.vacations.total_vacation_days"
            icon="mdi-calendar-multiple"
            color="green"
          />
          <StatCard
            title="الإجازات المرضية"
            :value="overview.vacations.total_sick_days"
            icon="mdi-medical-bag"
            color="red"
          />
          <StatCard
            title="المتوسط لكل موظف"
            :value="overview.vacations.average_per_employee"
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <StatCard
            title="إجمالي المواد"
            :value="overview.stock.total_items"
            icon="mdi-package-variant"
            color="blue"
          />
          <StatCard
            title="مواد منخفضة المخزون"
            :value="overview.stock.low_stock_items"
            icon="mdi-alert"
            color="yellow"
          />
          <StatCard
            title="مواد نفذت"
            :value="overview.stock.out_of_stock_items"
            icon="mdi-alert-circle"
            color="red"
          />
          <StatCard
            title="سندات الإدخال"
            :value="overview.stock.total_input_vouchers"
            icon="mdi-arrow-down-bold-box"
            color="green"
          />
          <StatCard
            title="سندات الصرف"
            :value="overview.stock.total_output_vouchers"
            icon="mdi-arrow-up-bold-box"
            color="purple"
          />
          <StatCard
            title="قيمة المخزون"
            :value="`${overview.stock.total_stock_value.toLocaleString('ar-EG')} د.ع`"
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            :value="overview.system.active_users"
            icon="mdi-account-check-outline"
            color="green"
          />
          <StatCard
            title="الأرشيف"
            :value="overview.system.total_archives"
            icon="mdi-archive"
            color="purple"
          />
          <StatCard
            title="النشاط الأخير (7 أيام)"
            :value="overview.system.recent_activity_count"
