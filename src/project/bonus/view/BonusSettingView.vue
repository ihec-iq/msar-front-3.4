...
const namePage = ref("Bonus.Setting");

 
 
const update = async () => {
  errors.value = null;
  const formData = prepareFormData(Setting.value);
  try {
    await SettingStore.update(Setting.value.Id.toString(), formData).then((response)=>{
