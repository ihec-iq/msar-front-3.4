...
const { employeeType } = storeToRefs(useEmployeeTypeStore());
...
const store = () => {
  ...
  EmployeeTypeStore
    ...
    .catch((error) => {
      ...
      errors.value = EmployeeTypeStore.getError(error);
      Swal.fire({
        icon: "error",
  ...
  await EmployeeTypeStore
    ...
    .then((response) => {
      if (response.status == 200) {
        ...
        employeeType.value.name = response.data.data.name;
      }
    })
    .catch((errors) => {
      console.log(errors);
...
const reset = () => {
  useEmployeeTypeStore().resetData();
};
</script>
<template>
