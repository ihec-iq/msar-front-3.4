...
import { useEmployeePositionStore } from "../employeePositionStore";
...
const namePage = ref("EmployeeType");
const route = useRoute();
const id = ref(Number(route.params.id));

const { employeePosition } = storeToRefs(useEmployeePositionStore());
...
const store = () => {
  ...
  EmployeePositionStore
    ...
    .catch((error) => {
      ...
      errors.value = EmployeePositionStore.getError(error);
      Swal.fire({
        icon: "error",
...
const showData = async () => {
  ...
  await EmployeePositionStore
    ...
    .then((response) => {
      if (response.status == 200) {
        employeePosition.value.id = response.data.data.id;
        employeePosition.value.name = response.data.data.name;
        employeePosition.value.level = response.data.data.level;
        employeePosition.value.code = response.data.data.code;
      }
    })
...
onMounted(async () => {
  ...
  if (Number.isNaN(id.value) || id.value === undefined) {
    namePage.value = t("EmployeeType.Add");
    employeePosition.value.id = 0;

    reset();
  } else {
    employeePosition.value.id = id.value;
    await showData();
...
const reset = () => {
  useEmployeePositionStore().resetData();
};
</script>
<template>
