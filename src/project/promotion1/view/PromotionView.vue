...
const ChangeDegreeStage = async () => {
  if (Promotion.value.DegreeStage.Degree) {
    await useBonusStore()
      .get_BonusJobTitle({
        bonusDegreeId: Promotion.value.DegreeStage.Degree.id,
      })
      .then((response) => {});
  }
};
const store = async () => {
  ...
  try {
    await PromotionStore.store(formData).then((response: any) => {
...
const update = async () => {
  ...
  const formData = prepareFormData(Promotion.value); // Updated reference
  try {
    await PromotionStore.update(Promotion.value.id, formData).then(
      (response: any) => {
        // Updated store method
        if (response.status === 200) {
          SuccessToast();
        ...
        <IRow cols-lg="4" cols-md="2" cols-sm="1">
          ...
          <ICol span="1" span-md="2" span-sm="4">
            ...
            <vSelect
              ...
              :options="useBonusStore().DegreeStages"
          ...
          <ICol span="1" span-md="2" span-sm="4">
            ...
            <vSelect
              ...
              :options="useBonusStore().Employees"
