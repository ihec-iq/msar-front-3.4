...
onMounted(async () => {
  ...
  await useInputVoucherStore().getAllItemsVSelectByEmployeeId(
    retrievalVoucher.value.Employee.id
  );
  ...
  <IPage :HeaderTitle="t(namePage)" :is-loading="isLoading">
      ...
      <IContainer>
        <IForm>
          <IRow cols-lg="4" cols-md="2" cols-sm="1">
            ...
            <ICol span="1" span-md="2" span-sm="1">
              <IInput :label="t('SignaturePerson')" name="InputVoucherNumber" v-model="retrievalVoucher.signaturePerson"
                :type="EnumInputType.Text" />
            </ICol>
          </IRow>
          <IFlex>
          ...
          <IFlex>
            ...
            <IBasis base="3/4" base-md="1/2" base-sm="1/2">
              <IInput :label="t('Notes')" name="InputVoucherNumber" v-model="retrievalVoucher.notes"
                :type="EnumInputType.Text" />
            </IBasis>
          </IFlex>
          <IRow>
          ...
          <IRow>
            <ICol>
              <ITable :items="retrievalVoucher.Items" :headers="headers" :showSearch="false" :showPrintButton="false"
                :showExportButton="false" :showColumnsButton="false" :showFilter="false" :showPagination="false">
    ...
    <IContainer class="w-full">
      <van-popup class="overflow-hidden dark:bg-darkNav" v-model:show="showPop" round position="bottom">
                ...
                <div class="rtl:text-right border-2 p-2 rounded-md">
                  ...
                  <cite>
                    ...
                    <div
                      ...
                      <b>{{ t("Item.In") }} : </b>
                      {{ Number(countIn) + Number(countReIn) }}
                      <b>{{ t("Item.Out") }} : </b>
                      {{ Number(countOut) + Number(countReOut) }}
                    </div>
        ...
        <IRow cols-lg="4" cols-xl="4" cols-md="2" cols-sm="1" v-if="VoucherItem.InputVoucherItem != null">
          ...
          <ICol :span="1" span-lg="1" span-xl="1" span-md="1">
            <IInput :label="t('Count')" :on-input="ChangeValueTotal" :type="EnumInputType.Number"
              v-model="VoucherItem.count" :max="Number(VoucherItem.InputVoucherItem.countIn) +
                Number(VoucherItem.InputVoucherItem.countReIn) -
                (Number(VoucherItem.InputVoucherItem.countOut) -
                  Number(VoucherItem.InputVoucherItem.countReOut))
                " :min="1" />
          </ICol>
          <ICol :span="1" span-lg="1" span-xl="1" span-md="1">
        ...
        <IContainer class="flex flex-row my-10">
          <IButton2 :text="t('Add')" color="blue" :variant="EnumButtonType.Default" :on-click="AddItem" v-if="IsAdd" />
          <IButton2 :text="t('Update')" color="blue" :variant="EnumButtonType.Default" :on-click="EditItem" v-else />
          <IButton2 class="" pre-icon="close-box" :text="t('Close')" color="blue" :variant="EnumButtonType.Text"
