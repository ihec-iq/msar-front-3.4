...
const store = () => {
  ...
  StockStore
    ...
    .catch((error) => {
      ...
      errors.value = StockStore.getError(error);
      Swal.fire({
        icon: "error",
  ...
  await StockStore
    ...
    .then((response) => {
      if (response.status == 200) {
        ...
        stock.value.name = response.data.data.name;
        stock.value.description = response.data.data.description;
      }
    })
    .catch((errors) => {
      console.log(errors);
