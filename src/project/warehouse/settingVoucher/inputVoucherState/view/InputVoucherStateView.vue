...
const namePage = ref("InputVoucherState");
const route = useRoute();
const id = ref(Number(route.params.id));

const { inputVoucherState } = storeToRefs(useInputVoucherStateStore());
...
const store = () => {
  ...
  InputVoucherStateStore
    ...
    .catch((error) => {
      ...
      errors.value = InputVoucherStateStore.getError(error);
      Swal.fire({
        icon: "error",
  ...
  await InputVoucherStateStore
    ...
    .then((response) => {
      if (response.status == 200) {
        ...
        inputVoucherState.value.name = response.data.data.name;
      }
    })
    .catch((errors) => {
      console.log(errors);
...
onMounted(async () => {
  ...
  if (Number.isNaN(id.value) || id.value === undefined) {
    namePage.value = t("Warehouse.InputVoucherState.Add");
    inputVoucherState.value.id = 0;
    reset();
  } else {
...
const reset = () => {
  useInputVoucherStateStore().resetData();
};
</script>
<template>
