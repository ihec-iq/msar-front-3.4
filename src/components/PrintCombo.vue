...
function splitTable(tableEl: HTMLTableElement, maxRows: number): string[] {
  ...
  for (let i = 0; i < rows.length; i += maxRows) {
    const slice = rows
      ...
      .join("");
    chunks.push(
      `<table border="1" style="border-collapse:collapse;width:100%;">${thead}<tbody>${slice}</tbody></table>`
    );
  }
  return chunks;
}
    ...
    <div style="position: relative">
      ...
      <transition name="fade-slide">
        ...
        >
          ...
          <div
            v-for="opt in props.printOptions"
            :key="opt.elementId"
            style="margin: 6px 0"
          >
            <label
          ...
          <label><input type="checkbox" v-model="includeHeader" /> هيدر</label
          ><br />
          <label
            ><input type="checkbox" v-model="includePageFooter" /> فوتر
            الصفحة</label
          ><br />
          <label
            ><input type="checkbox" v-model="includePageNumbers" /> ترقيم</label
          ><br />
