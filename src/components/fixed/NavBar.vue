...
import { IconLink, Links } from "./FixedMenu";
...
<template>
  <div class="bg-sideNav dark:bg-[#2c3147] h-full ">
    ...
    <div class="lg:hidden  fixed top-0 ltr:left-3 rtl:right-1 z-[1001]">
      <button @click="toggleMobileMenu" class="p-2 rounded-xl shadow bg-white/90 dark:bg-[#2b2f42]"
        aria-label="Open menu" aria-controls="mobile-drawer" :aria-expanded="showMobileMenu">
      ...
      <nav class="flex pt-3 flex-col">
        <ul class="relative overflow-hidden">
          <li v-for="link in filteredLinks" :key="link.routerName" class="overflow-hidden flex items-center">
            ...
            <div v-if="tab === link.tab && checkPermission(link.permissions?.[0] ?? 'public')"
    ...
    <transition name="fade">
      <div v-show="showMobileMenu" class="lg:hidden fixed inset-0 z-[1000]" @click.self="closeMobileMenu">
        <!-- overlay -->
        <div class="absolute inset-0 bg-black/50" @click="showMobileMenu = false"></div>
        <!-- drawer -->
        <transition name="slide-left">
          <aside id="mobile-drawer" v-show="showMobileMenu"
            ...
            <div class="flex items-center justify-between p-3">
              <div class="flex items-center gap-2">
                <img src="@/assets/ihec-logo.jpg" class="w-10 h-10 rounded-full" alt="Logo" />
                <span class="text-text dark:text-textLight font-medium truncate max-w-[9rem]">
                  {{ user?.Employee?.name }}
                </span>
              </div>
              <button @click="closeMobileMenu" class="p-2 rounded-lg" aria-label="Close menu">
            ...
            <nav class="px-2 pb-6">
              <ul>
                <li v-for="link in filteredLinks" :key="link.routerName" class="flex items-center">
                  <router-link :to="{ name: link.routerName }"
                    ...
                    :title="link.title" @click="tab = link.tab">
                    <Icon v-if="link?.mdi" :icon="link.mdi" class="text-[20px]" />
                    <span v-else v-html="link.icon" class="text-[20px]"></span>
                    <span class="flex-1 text-sm text-left">{{ link.title }}</span>
                  </router-link>
                </li>
              </ul>
              ...
              <!-- optional: show description when allowed -->
              <template v-for="link in filteredLinks" :key="link.routerName + '-desc-mobile'">
                <div v-if="tab === link.tab && checkPermission(link.permissions?.[0] ?? 'public')"
