...
export async function printTable(
  ...
  headers.forEach((h, i) => {
    if (h.print === false)
      hiddenIdx.push(
        i + (opts.showRowNumber ? 1 : 0) + (opts.selectable ? 1 : 0)
      );
  });

  const headerRow = clone.querySelector("thead tr");
  if (headerRow)
    [...hiddenIdx].reverse().forEach((i) => headerRow.children[i]?.remove());
  const bodyRows = clone.querySelectorAll("tbody tr");
  bodyRows.forEach((r) =>
    [...hiddenIdx].reverse().forEach((i) => r.children[i]?.remove())
  );

  const popup = window.open("", "_blank");
  if (!popup) return;
  popup.document
    .write(`<!doctype html><html><head><title>${opts.title || "Table"}</title>
    <style>
