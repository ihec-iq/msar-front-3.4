...
const props = defineProps({
  ...
  pageSizeOptions: { type: Array as () => number[], default: () => [10, 25, 50, 100] },
...
// ========== Columns visibility ==========
const initialVisible = props.headers.filter(h => h.visible !== false).map(h => h.value)
const { visibleColumns, toggleAll } = useColumnPersistence(props.persistKey || props.title, initialVisible)
...
const displayHeaders = computed<ITableHeader[]>(() => {
  ...
  return visibleColumns.value.map(k => map.get(k)).filter((h): h is ITableHeader => !!h)
})
...
function asStrictNumber(v: unknown): number | null {
  ...
  const normalized = s.replace(/\s+/g, '').replace(/,/g, '').replace(/，/g, '').replace(/٬/g, '').replace(/٫/g, '.')
...
function smartCompare(a: unknown, b: unknown): number {
  ...
  if (aNull) return 1
  if (bNull) return -1
  const at = asTime(a), bt = asTime(b)
  if (at != null && bt != null) return at - bt
  const an = asStrictNumber(a), bn = asStrictNumber(b)
...
const clientSorted = computed<ITableItem[]>(() => {
  ...
  return [...baseRows.value].sort((a, b) => dir * smartCompare(getByPath(a, key), getByPath(b, key)))
})
...
const to = computed(() => Math.min(core.page.value * core.pageSize.value, clientSorted.value.length))

// تحديث حالة الفرز عند النقر
function onSort(header: ITableHeader) {
  if (header.sortable === false) return
  const key = header.sortKey || header.value
  uiSort.value = uiSort.value.key === key ? { key, asc: !uiSort.value.asc } : { key, asc: true }
  if (!props.serverSide) { core.setPage(1); emit('update:currentPage', 1) }
...
function getRowKey(row: ITableItem, i: number): RowKey {
  ...
  if (typeof props.rowKey === 'string' && props.rowKey) return (row as any)[props.rowKey] ?? ensureRowId(row)
...
// ========== أدوات ==========
function onSearch(q: string) { core.setSearch(q); emit('search-change', q) }
  ...
  <div class="table-container my-2">
    <div v-if="items.length">
      <ITableToolbar
        ...
        @update:page-size="(s:number)=>{ core.setPageSize(s); $emit('update:pageSize', s); $emit('update:currentPage', 1) }"
        @search="onSearch"
        @export="onExport"
        @print="onPrint"
        @toggle-all-columns="toggleAll(headers.map(h=>h.value))"
        @update:visible-columns="(cols:string[])=>{ visibleColumns = cols }"
      >
      ...
      >
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-750 border-b border-gray-200 dark:border-gray-600">
              <th v-if="selectable" class="px-4 py-3 text-center text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border-r border-gray-200 dark:border-gray-600" :class="{ 'sticky top-0 z-10 bg-gray-100 dark:bg-gray-700': stickyHeader }">
                <input type="checkbox" :checked="selection.allOnPage.value" @change="selection.toggleAllOnPage" aria-label="Select all on page" />
              </th>
              <th v-if="showRowNumber" class="px-4 py-3 text-center text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider border-r border-gray-200 dark:border-gray-600 bg-gray-100 dark:bg-gray-700" :class="{ 'sticky top-0 z-10 bg-gray-100 dark:bg-gray-700': stickyHeader }">#</th>
              ...
              >
                <button
                  ...
                  :class="h.sortable===false ? 'cursor-default' : 'cursor-pointer hover:opacity-80'"
                  @click.stop.prevent="onSort(h)"
                >
                  <span class="text-xs font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider">
                    {{ h.caption }}
                    <span v-if="h.print===false" class="ml-1 text-gray-400 dark:text-gray-500 print-hide" title="Hidden on print">
                      <svg class="w-4 h-4 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                    </span>
                  </span>
                  <span v-if="uiSort.key === (h.sortKey || h.value)" class="text-blue-600 dark:text-blue-400 font-bold text-sm">
                    {{ uiSort.asc ? '↑' : '↓' }}
                  </span>
                </button>
              </th>
          ...
          <tbody :key="sortRenderKey" class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr
            ...
            >
              <td v-if="selectable" class="px-4 py-3 text-center text-sm text-gray-500 dark:text-gray-400 font-medium border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                <input
                  type="checkbox"
                  :checked="selection.selected.value.includes(getRowKey(row, i))"
                  @change="selection.toggleOne(row, i)"
                  aria-label="Select row"
                />
              </td>

              <td v-if="showRowNumber" class="px-4 py-3 text-center text-sm text-gray-500 dark:text-gray-400 font-medium border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 group-hover:bg-blue-100 dark:group-hover:bg-gray-600/50">
              ...
              <td
                ...
                :class="[ dense ? 'py-2' : 'py-3', h.align==='center' ? 'text-center' : h.align==='right' ? 'text-right' : 'text-left' ]"
              >
                <slot :name="h.value" :row="row" :value="getByPath(row, h.value)">
                  <template v-if="h.format">{{ h.format(row, getByPath(row, h.value)) }}</template>
                  <template v-else>{{ getByPath(row, h.value) as any }}</template>
                </slot>
              </td>
            </tr>
          </tbody>
      ...
      <ITablePagination
        ...
        @prev="() => { core.setPage(core.page.value - 1); $emit('update:currentPage', core.page.value) }"
        @next="() => { core.setPage(core.page.value + 1); $emit('update:currentPage', core.page.value) }"
      />
      <slot v-else name="serverPager" />
    </div>

    <div v-else-if="!items.length && showNoData" class="text-center mt-8 py-8 text-gray-500 dark:text-gray-400">
      <div class="text-lg">{{ t(titleNoData) || t('noData') }}</div>
    </div>

    <div v-else-if="!baseRows.length" class="text-center py-4 text-gray-500 dark:text-gray-400 border-2 border-dotted border-gray-400 rounded-lg">
      <div class="text-lg">{{ t(titleNoData) || 'No results found for your search' }}</div>
    </div>
  </div>
</template>
...
.dark .bg-gradient-to-r.from-gray-50.to-gray-100{background:linear-gradient(to right,rgb(31,41,55),rgb(17,24,39))}
.sticky-first-col{position:sticky;left:0;z-index:5;background:inherit}
@media print{
  .table-container,.table-container *{visibility:hidden}
  #printable-table,#printable-table *{visibility:visible}
  #printable-table{position:absolute;left:0;top:0;width:100%;overflow:visible!important;border:none!important;border-radius:0!important}
