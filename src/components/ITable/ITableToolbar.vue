// ─────────────────────────────────────────────────────────────────────────────
// File: ITableToolbar.vue // Search + actions (export/print) + column popover.
Emits small, focused events. //
─────────────────────────────────────────────────────────────────────────────
  ...
  <div class="mb-4 flex flex-wrap justify-between items-center gap-4">
    <div class="flex items-center gap-3">
      ...
      <div v-if="showColumnsButton" class="relative">
        <details class="relative">
          ...
          <div
            class="absolute top-full left-0 mt-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl z-50 min-w-72 p-5 backdrop-blur-sm"
          >
            <div class="mb-4 flex justify-between items-center">
              <span class="text-sm text-gray-600 dark:text-gray-400"
                >Visible {{ visibleColumns.length }} of
                {{ headers.length }}</span
              >
              <button
                @click="$emit('toggle-all-columns')"
            ...
            <div class="space-y-1 max-h-64 overflow-y-auto custom-scrollbar">
              ...
              >
                <input
                  type="checkbox"
                  :value="h.value"
                  :checked="visibleColumns.includes(h.value)"
                  @change="
                    $emit(
                      'update:visibleColumns',
                      visibleColumns.includes(h.value)
                        ? visibleColumns.filter((v) => v !== h.value)
                        : [...visibleColumns, h.value]
                    )
                  "
    ...
    <div class="flex items-center gap-3">
      ...
      <button
        ...
        @click="$emit('export')"
        class="bg-green-100 hover:bg-green-200 disabled:opacity-50 dark:bg-green-900 dark:hover:bg-green-800 text-green-700 dark:text-green-200 px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-200 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800"
      >
        <svg
          class="w-4 h-4"
          fill="none"
      ...
      <button
        ...
        @click="$emit('print')"
        class="bg-blue-100 hover:bg-blue-200 dark:bg-blue-900 dark:hover:bg-blue-800 text-blue-700 dark:text-blue-200 px-4 py-2 rounded-lg flex items-center gap-2 transition-all duration-200 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2 dark:focus:ring-offset-gray-800"
      >
        <svg
          class="w-4 h-4"
          fill="none"
